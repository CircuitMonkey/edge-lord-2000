...
int8_t ModeCross::buttonState(uint8_t _s) {
...
uint8_t ModeCross::getMotorVal(uint8_t _n ) {
  if ( stopped ) return 0;

  uint8_t col = _n%2;
  if ( _n > 1 ) { // lower, swap left right
    col = 1-col;
  }
  return animTable[animStep][col] * slider[_n/2]->getVal() / 16;
